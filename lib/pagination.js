...
class Pagination extends Ctx {
    ...
    config(opts) {
        ...
        this._ctxPage = this.ctx
                        && this.ctx[this._pageQuery]
                        && this.ctx[this._pageQuery][this._pageKey]
                        && +this.ctx[this._pageQuery][this._pageKey];
    ...
    // 设置当前页
    page(page) {
        this._page = (page && +page) || this._ctxPage || 1;
        return this;
    }
    ...
    render(total, page, opts) {
        ...
        return this.opts['pageRender'].replace(/\$\{(\w+)\}/g, (...args) => {console.log('test', args[1]);
            return this[args[1]]();
        });
    }
