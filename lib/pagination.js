...
class Pagination extends Context {
    ...
    // 分页配置
    init(options) {
        this.options = {...cfg_Page, ...this.options, ...options};
    ...
    // 解析网址
    _parseUrl(page) {
        if(this.options.urlPage == '') {
            const url = this.ctx.url;
            const urls = url.split('?');
            if(this.ctxPage) {
                if(this.options.pageType == 'params') {
                    this.options.urlPage = urls[0].replace(/(\d+)/g, (...args) => {
                        return args[1] == this.curPage ? '${page}' : args[1];
                    }) + (urls[1] ? ('?' + urls[1]) : '');
                } else {
                    this.options.urlPage = url.replace(this.options.pageKey + '=' + this.curPage, this.options.pageKey + '=${page}');
                }
            } else {
                if(this.options.urlIndex == '') {
                    this.options.urlIndex = this.ctx.url;
                }
                if(this.options.pageType == 'params') {
