...
class Db extends Context {
    ...
    connect(options='default') {
        ...
        if(!cur_pool) {
            ...
            logger.sql('连接池创建成功！', `连接池：${pool.size}`);
        }
        ...
        return new Promise((resolve, reject) => {
            p.getConnection((err, connection) => {
                if(err) {
                    ...
                    resolve(connection);
                    logger.sql('数据库建立连接成功！');
                    logger.sql(`连接数：limit(${p.config.connectionLimit}) all(${p._allConnections.length}) acquiring(${p._acquiringConnections.length}) free(${p._freeConnections.length}) queue(${p._connectionQueue.length})`);
                }
            });
        });
    }
    ...
    async query(sql, params) {
        ...
        return new Promise((resolve, reject) => {
            conn.query(sql, params, (err, data) => {
                if(err){
                    err.msg = '数据操作失败，SQL：' + this.format(sql, params);
                    logger.error(err.msg);
                    reject(err);
                }else{
                    logger.sql('数据操作成功，SQL：' + this.format(sql, params));
                    resolve(data);
                }
                if(!trans.get(this.ctx)) {
                    conn.release();
                    logger.sql('数据库连接释放成功！');
                }
            });
        });
    }
