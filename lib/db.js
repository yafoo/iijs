...
class Db extends Ctx {
        ...
        return new Promise((resolve, reject) => {
            p.getConnection((err, connection) => {
                if(err) {
                    ...
                    logger.log(`连接数：limit(${p.config.connectionLimit}) all(${p._allConnections.length}) acquiring(${p._acquiringConnections.length}) free(${p._freeConnections.length}) queue(${p._connectionQueue.length})`);
                }
            });
        });
    }
    ...
    async select(condition) {
        ...
        if(this._sql.cache) {
            ...
            if(Db.getCache(cache_key)) {
                this.reset();
                return Db.getCache(cache_key);
            }
            const result = await this.query(this._queryStr, params);
            Db.setCache(cache_key, result, cache_time);
            return result;
        }
    ...
    setCache(key, data, cache_time) {
        Db.setCache(key, data, cache_time);
    }
    ...
    setInterval(delay_time) {
        Db.setInterval(delay_time);
    }
}
...
Db.setCache = function(key, data, cache_time) {
    const now_time = Math.round(new Date() / 1000);
    cache[key] = {data: data, time: cache_time + now_time};
}

Db.delCache = function(key) {
    if(key) {
        delete cache[key];
    } else {
...
Db.setInterval = function(delay_time) {
    ...
    timer = setInterval(() => {
        const now_time = Math.round(new Date() / 1000);
        for(let key in cache) {
            cache[key].time < now_time && (delete cache[key]);
        }
        logger.log('数据库缓存定时自动清理！');
    }, delay_time * 1000);
}

Db.setInterval(20 * 60 * 60);
