...
class View {
    ...
    parse(template){
        ...
        if(path.extname(template)){
            ...
            switch(temp.length){
                case 1:
                    view_file = this.ctx._app + '/view/' + this.ctx._controller + cfg_app.view_depr + temp[0];
                    break;
                case 2:
                    view_file = this.ctx._app + '/view/' + temp[0] + cfg_app.view_depr + temp[1];
                    break;
                case 3:
        ...
        view_file = path.join(this.ctx.loader.$path, './', view_file);
        return view_file;
    }
