async function run(ctx, next, type='controller') {
    ...
    const config = (loader.config && loader.config.app) || require('./config');
    ...
    if(type == 'view' && config.view_depr != '/') {
        //输出模版文件内容
        const content = await loader[ctx._app][type].readFile(ctx._controller + config.view_depr + ctx._action + config.view_ext);
    ...
    if(type == 'view') {
        //输出模版文件内容
        const content = await loader[ctx._app][type].file(ctx._action + config.view_ext);
        if(!content && config.app_debug) {
