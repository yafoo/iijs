...
async function run(ctx, next, type=cfg_app.controller_folder) {
    ...
    if (typeof ctx.$iijs[ctx.$app][type][ctx.$controller][ctx.$action] != 'function') {
        if (cfg_app.app_debug) {
    ...
    //执行控制器方法
    if(typeof ctx.$iijs[ctx.$app][type][ctx.$controller]['_init'] == 'function') {
        await ctx.$iijs[ctx.$app][type][ctx.$controller]['_init']();
    }
    await ctx.$iijs[ctx.$app][type][ctx.$controller][ctx.$action]();
}

function setLoader(ctx, next) {
    ctx.$iijs = require('noader')(path.dirname(module.parent.parent.parent.parent.filename), ctx, next);
