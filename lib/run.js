...
async function run(ctx, next, type=cfg_app.controller_folder) {
    // 设置根加载器
    ctx.$iijs = noader(path.dirname(module.parent.parent.parent.parent.filename), ctx, next);
    ...
    if (ctx.$app[0] == '_' || ~cfg_app.deny_apps.indexOf(ctx.$app) || !ctx.$iijs[ctx.$app]) {
        if (cfg_app.app_debug) {
    ...
    if(type == cfg_app.view_folder) {
        // const content = await readFile(path.join(ctx.$iijs[ctx.$app][type].$prop.path, ctx.$controller + cfg_app.view_depr + ctx.$action + cfg_app.view_ext));
        ...
        // ctx.body = content;
        try {
            await send(ctx, path.join(ctx.$iijs[ctx.$app][type].$prop.path, ctx.$controller + cfg_app.view_depr + ctx.$action + cfg_app.view_ext), {root: '/'});
        } catch (err) {
            if (err.status == 404 && cfg_app.app_debug) {
                ctx.body = `模板文件:${ctx.$app}/${type}/${ctx.$controller + cfg_app.view_depr + ctx.$action + cfg_app.view_ext}不存在！`;
                logger.log(ctx.body);
            }
        }
    ...
    const controller = ctx.$iijs[ctx.$app][type][ctx.$controller] || ctx.$iijs[ctx.$app][type]['empty'];

    if (ctx.$controller[0] == '_' || ctx.$action[0] == 'empty' || !controller) {
        if (cfg_app.app_debug) {
    ...
    if (ctx.$action[0] == '_' || ctx.$action[0] == 'empty' || typeof controller[$action] != 'function') {
        if (cfg_app.app_debug) {
